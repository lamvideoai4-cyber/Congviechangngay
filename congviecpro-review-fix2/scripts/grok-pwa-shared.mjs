...
export function normalizeHeadContext(ctx = {}) {
  ...
  // An explicit appName belongs to the PWA chrome (for example the app
  // title shown on iOS). Do not let the workspace's baked OG site title
  // override it. OG title resolution is still handled separately below so
  // a document <title> can remain the share-card title.
  const explicitAppName = String(ctx.appName ?? "").trim();
  const appName =
    explicitAppName ||
    String(site.title ?? "").trim() ||
    appNameFromHost(ctx.host ?? "") ||
    DEFAULT_APP_NAME;
  return {
    appName,
