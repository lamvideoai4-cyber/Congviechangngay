...
function parseGrid(
  ...
  headers.forEach((h, i) => {
    if (matchKey(h, SKIP_HEADERS)) return;
    if ([cols.sbd, cols.name, cols.position, cols.phone, cols.group, cols.shift, cols.note, cols.stt, cols.date, cols.code].includes(i)) {
      return;
    }
...
export function buildPersonnelTemplate(): Blob {
  ...
  const peopleRows = [
    peopleHeader,
    ["1", "00001", "Nguyễn Văn An", "Leader", "0900000001", "Tổ thành phẩm E", "M", "Ví dụ — xóa khi nhập ca thật"],
    ["2", "00002", "Trần Thị Bình", "Đóng Cont,IGHS", "0900000002", "Tổ thành phẩm E", "X", ""],
    ["3", "00003", "Lê Văn Cường", "Lái Xe,IGHS", "0900000003", "Tổ thành phẩm E", "A", ""],
    ["4", "00004", "Phạm Thị Dung", "Cấp,nhận hàng", "0900000004", "Tổ thành phẩm E", "D", "Ca đêm"],
  ];
  ...
  const wide = [
    ...
    wideHeader,
    ["1", "00001", "Nguyễn Văn An", "Leader", "0900000001", "M", "M", "M", "P", "M", "M", "M"],
    ["2", "00002", "Trần Thị Bình", "Đóng Cont,IGHS", "0900000002", "X", "X", "X", "S", "X", "X", "X"],
    ["3", "00003", "Lê Văn Cường", "Lái Xe,IGHS", "0900000003", "A", "A", "A", "SA", "A", "A", "A"],
    ["4", "00004", "Phạm Thị Dung", "Cấp,nhận hàng", "0900000004", "D", "D", "D", "E", "D", "D", "O"],
  ];

  const codes = [
    ["Mã", "Tên", "Bắt đầu", "Kết thúc", "Loại", "Giờ ca", "Dùng cho"],
    ...SHIFT_CATALOG.map((s) => [
      s.code,
      s.label,
      s.startTime,
      ...
      s.plannedMinutes / 60,
      s.kind === "WORK" ? "Chấm công / AMH" : "Khai ca nghỉ",
    ]),
  ];

  const guide = [
    ["Hướng dẫn nhập nhanh nhân sự + lịch ca"],
    [""],
    ["1. Sheet Nhân sự: mỗi dòng một người. SBD là khóa (trùng SBD thì cập nhật)."],
    ["2. Sheet Lịch ngang: giống file Excel tổ — cột ngày 1/7, 2/7… ô chứa mã ca M, X, A, D, P, O…"],
    ["3. Để trống ô ngày = chưa xếp ca ngày đó."],
    ["4. Ca làm: M 06:00–14:00 (8h), M1 06:00–15:00 (9h), X5 07:00–16:00 (9h), X 08:00–17:00 (9h), X3 09:00–18:00 (9h), A 14:00–22:00 (8h), D 22:00–06:00 (8h, qua ngày)."],
    ["5. Nghỉ: SM/SM1/S/SA/E = ngày nghỉ theo khung ca; P phép; CK nghĩa vụ; RO không lý do; TS thai sản; O ốm."],
    ["6. Chấm công: tích Đã đến. Đến sau giờ bắt đầu ca → ghi phút muộn."],
    ["7. AMH = giờ ca đã xác nhận + OT đã xác nhận + điều chỉnh đã duyệt."],
    ["8. OT là làm thêm ngoài giờ ca."],
    ["9. Không điền số điện thoại thật vào file mẫu này — thay bằng dữ liệu tổ."],
    [""],
    ["Có thể nhập file lịch hiện tại (.xlsx) nếu có cột SBD và Họ và Tên."],
    [`Các cột ngày không ghi năm sẽ lấy năm ${y} (đổi năm ở dòng tiêu đề).`],
    ...dates.map((d, i) => [`Cột ${dateHeaders[i]} = ${d}`]),
  ];

  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(peopleRows), "Nhân sự");
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(wide), "Lịch ngang");
